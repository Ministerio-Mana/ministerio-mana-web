...
-- =========================================================
-- Q5) Integridad de scope para roles locales y nacionales
-- =========================================================
...
select
  ...
  case
    when p.role = 'national_pastor' and coalesce(p.country, '') = '' then 'NATIONAL_SIN_COUNTRY'
...
where
  (p.role = 'national_pastor' and coalesce(p.country, '') = '')
  or (
...
select
  ...
  e.created_at,
  case
    when e.scope = 'LOCAL' and e.church_id is null then 'LOCAL_SIN_CHURCH_ID'
    when e.scope = 'NATIONAL' and coalesce(e.country, '') = '' then 'NATIONAL_SIN_COUNTRY'
    when e.scope = 'GLOBAL' and (e.church_id is not null or coalesce(e.country, '') <> '') then 'GLOBAL_CON_SCOPE_RESIDUAL'
    else null
  end as issue
from public.events e
where
  (e.scope = 'LOCAL' and e.church_id is null)
  or (e.scope = 'NATIONAL' and coalesce(e.country, '') = '')
  or (e.scope = 'GLOBAL' and (e.church_id is not null or coalesce(e.country, '') <> ''))
order by e.created_at desc;

-- =========================================================
-- Q9) Eventos nacionales creados por nacional fuera de su pais
-- =========================================================
select
  e.id,
  e.title,
...
where e.scope = 'NATIONAL'
  and p.role = 'national_pastor'
  and coalesce(e.country, '') <> coalesce(p.country, '')
order by e.created_at desc;

-- =========================================================
-- Q10) Cobertura de perfiles para contactos Cumbre
...
-- =========================================================
-- Q11) Bookings portal-iglesia sin church_id
-- =========================================================
select
  b.id,
  b.created_at,
...
-- =========================================================
-- Q12) Calidad de user_profiles vs auth.users
-- =========================================================
select
  lower(p.email) as email,
  p.role,
...
-- =========================================================
-- Q13) Local roles sin membership activa en su iglesia de perfil
-- =========================================================
with active as (
  select
    cm.user_id,
...
-- =========================================================
-- Q14) Integridad de cuotas Cumbre (planes e installments)
-- =========================================================
select
  pp.id as plan_id,
  pp.booking_id,
...
-- =========================================================
-- Q15) Diagnostico dirigido por correos reportados
-- =========================================================
