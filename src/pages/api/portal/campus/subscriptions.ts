...
import {
  getCampusSubscriptionByIdForUser,
  resolveStripePeriod,
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const subscription = await getCampusSubscriptionByIdForUser(id, user.id);
  if (!subscription) {
  ...
  if (action === 'manage') {
    if (subscription.provider !== 'stripe' || !subscription.provider_customer_id) {
      return json({ ok: false, error: 'Este aporte se gestiona desde el portal Maná.' }, 400);
    }
  ...
  if (action === 'pause') {
    ...
    if (subscription.provider === 'stripe' && subscription.provider_subscription_id) {
      ...
      period = resolveStripePeriod(stripeSub);
    }

    const updated = await updateCampusSubscriptionById(subscription.id, {
      status: 'PAUSED',
  ...
  if (action === 'resume') {
    ...
    if (
      ...
      return json({ ok: false, error: 'Esta siembra todavía no tiene fuente de pago automática en Wompi.' }, 400);
    }
