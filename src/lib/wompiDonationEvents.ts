...
export async function processWompiDonationTransaction(params: {
  ...
  if (!campusSubscription) return { processed: true, reference, status };

  let sourceId = campusSubscription.provider_payment_source_id || campusSubscription.provider_payment_method_id || null;
  ...
  if (status === 'APPROVED' && paymentSourceId) {
    ...
    && paymentMethodToken
    && campusSubscription.donor_email
  ) {
    try {
      sourceId = await createWompiPaymentSource({
        token: String(paymentMethodToken),
        customerEmail: campusSubscription.donor_email,
      });
    } catch (error: any) {
      ...
      void logSecurityEvent({
        ...
        detail: sourceError,
        meta: { campusSubscriptionId: campusSubscription.id, reference },
      });
    }
  }
  ...
  await updateCampusSubscriptionById(campusSubscription.id, {
    status: status === 'APPROVED'
      ...
      : status === 'FAILED'
        ? 'PAYMENT_FAILED'
        : campusSubscription.status,
    provider_payment_source_id: sourceId || campusSubscription.provider_payment_source_id,
    provider_payment_method_id: sourceId || campusSubscription.provider_payment_method_id,
    current_period_start: status === 'APPROVED' ? periodStart.toISOString() : campusSubscription.current_period_start,
    current_period_end: status === 'APPROVED' ? periodEnd : campusSubscription.current_period_end,
    next_charge_at: status === 'APPROVED' && sourceId ? periodEnd : status === 'FAILED' ? null : campusSubscription.next_charge_at,
