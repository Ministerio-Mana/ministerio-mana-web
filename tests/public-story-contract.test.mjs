...
test('Peticiones encaja papeles en ranuras reales y mantiene las peticiones accesibles', async () => {
  ...
  assert.match(logic, /const PAGE_SIZE_COMPACT = 5/);
  assert.match(logic, /const PAGE_SIZE_TABLET = 6/);
  assert.match(logic, /const PAGE_SIZE_DESKTOP = WALL_SLOTS\.length/);
  assert.match(logic, /isCompact\(root\) \|\| mobileWallQuery\.matches/);
  assert.match(logic, /createDecorativePaperSlot\(position, index\)/);
  assert.match(logic, /slots\.forEach\(\(position, index\) =>/);
  ...
  assert.match(view, /reservedPaperSlots\.some/);
